...
/**
 * Created by IntelliJ IDEA. User: Admin Date: 19.11.12 Time: 10:44
 */
public class ExcelReportTest
{
	...
	public void testCreate() throws Exception
	{
		...
		list.add(new GetterTestClass(
				"test1 is test test1 is test test1 is test test1 is test test1 is test test1 is test test1 is test " +
				"test1 is test test1 is test",
				1));
		...
		{
			HSSFCell cell = headerRow.getCell((short) cellId++);
		...
		Assert.assertEquals("1", row.getCell((short) 0).getStringCellValue());
		Assert.assertEquals("false", row.getCell((short) 1).getStringCellValue());
		Assert.assertEquals("test1 is test test1 is test test1 is test test1 is test " +
		                    "test1 is test test1 is test test1 is test " +
		                    "test1 is test test1 is test",
				row.getCell(
		...
		Assert.assertEquals("11", row.getCell((short) 0).getStringCellValue());
		Assert.assertEquals("false", row.getCell((short) 1).getStringCellValue());
		Assert.assertEquals("test2", row.getCell((short) 2).getStringCellValue());

		Assert.assertTrue(repFile.delete());
	}
}
